...
struct StandardTokenStreamDecoder: TokenStreamDecoder {
    ...
    mutating func finish(emit: (TokenStreamEvent) -> Bool) -> Bool {
        // The fork's pendingOutput drain now lives in ToolCallProcessor.processEOSOutputs(),
        // so this method stays byte-identical to upstream. wangqi modified 2026-08-31.
