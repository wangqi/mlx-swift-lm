...
public class Qwen2VL: Module, VLMModel, KVCacheDimensionProvider {
        ...
        -> PrepareResult
    {
        ...
        // wangqi modified 2026-07-03 (hybrid chunk merge): chunked prefill on iOS / Catalyst avoids
        // the [1, h, N, N] attention abort on large prompts. Only the TEXT-ONLY path is chunked — it
        // has no M-RoPE spatial positions (inputEmbeddings returns positionIds == nil there), so the
        // language model falls back to sequential RoPE keyed off the cache offset per chunk, which is
        // correct across chunks. The image/video path carries M-RoPE spatial positions (upstream
        // PR #345) that cannot be chunked without slicing positionIds in lockstep, so it falls
        // through to the single-shot path below.
        if allPixels == nil {
            return chunkedVLMPrefill(
                ...
                return self.languageModel(nil, cache: cache, state: nil, inputEmbedding: embChunk)
            }
        }
#endif
        ...
        //
        // NOTE: single-shot prefill. Mirroring Qwen25VL.swift, M-RoPE feeds
        // `positionIds` ([3, batch, seq]) through decoder state; chunking the
        // embeddings without slicing positionIds in lockstep would feed wrong
        // positions to M-RoPE. On iOS/Catalyst the text-only path is chunked
        // above (positionIds nil there); the image/video path and all of macOS
        // reach this single-shot path.
