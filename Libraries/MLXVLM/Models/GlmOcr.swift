...
public class GlmOcr: Module, VLMModel, KVCacheDimensionProvider {
        ...
        -> PrepareResult
    {
        ...
        // Pre-compute full-prompt M-RoPE positions via LMOutput.State so the
        // language model can slice by cacheOffset on every (chunked or single)
        // call. Required for iOS chunked prefill: the per-chunk callAsFunction
        // would otherwise recompute text-only positions without imageGridTHW.
        // Upstream moved this off of mutable model properties in PR #283. —
